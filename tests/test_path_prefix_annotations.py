...
`ParamMeta.path_prefix` (stimela-ninja #78) marks a `str` input that names a
...
def _marked() -> set[tuple[str, str]]:
    return {(c.name, f) for c in _binary_cabs() for f, m in c.field_meta.items() if m.path_prefix}
        ...
        for field, meta in cab.field_meta.items():
            if meta.path_prefix:
                assert field not in paths, (
                    f"{cab.name}.{field} is marked path_prefix but path-typed"
                )
...
def test_every_marked_stem_resolves_a_write_directory():
    ...
    """
    for cab in _binary_cabs():
        stems = [f for f, m in cab.field_meta.items() if m.path_prefix]
        if not stems:
            continue
...
def test_no_unmarked_string_stem_remains():
    ...
    for cab in _binary_cabs():
        ...
        for ref in referenced:
            ...
            if meta is None or not meta.path_prefix:
                missed.append((cab.name, ref))
    # ddfacet's cache_dir/montblanc_log_file and killms' DDF cache dir are
    # string write *paths* rather than product stems -- see the module
    # docstring of this file's PR. Listed explicitly so adding a genuinely new
    # one is a failure rather than a silent addition to an allowlist.
    assert set(missed) == {
        ("ddfacet", "cache_dir"),
...
def test_the_check_actually_fires():
    ...
    with pytest.raises(ValueError, match="marked path_prefix but named by no write declaration"):
        Cab(
            ...
            field_meta={"stem": ParamMeta(path_prefix=True)},
        )
